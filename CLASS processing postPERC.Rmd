...
code to pull out students who took it more than once
```{r eval=FALSE, include=FALSE}
twice_taken <- temp2 %>% group_by(student_id) %>% mutate(take_twice = length(student_id))
twice_taken <- twice_taken %>% filter (take_twice>=2)
save(twice_taken, file="CLASS_taken_multiple_times.Rdata")
```
...
```{r}
dredge_data <- temp2[,c("ID","course_id", "course_use_las" ,"collab_learning_during_class" ,"student_id",                         "gender_text","male" ,"female","transgender","gender_noanswer","gender_other",
                        "hispanic" ,"white" , "black","asian", "american_indian" , "hawaiian_or_other_pacific_islander", "race_noanswer" , "race_other" , "year_in_school"  , "first_time"  , "l_pre"  , "e_pre"  , "l_post" , "e_post"    )]
str(dredge_data)

save(dredge_data, file= "dredge_data.RData") 
```

```{r}
course_descriptions <- subset(temp, Physics==1)
length(unique(dredge_data$course_id))
course_descriptions <- unique(course_descriptions[c(139:147)])
length(unique(course_descriptions$course_id))

course_descriptions_7_10 <- readxl::read_excel("course_descriptions_7_10.xlsx", sheet = "course_descriptions_7_10 (2)")
course_descriptions <- left_join(course_descriptions, course_descriptions_7_10, by="course_id")
write.csv(course_descriptions, file = "course_descriptions_11_5_19.csv")
